...
COMMAND_HELP = "Usage:".green + " #{$0}".light_blue + " [-h] [-c] [-f [file.json]] [-r] [-u]\n" +
...
def check_hash(file, hash, algorithm)
  digest = ''
  if algorithm.eql? 'sha-1'
    digest = Digest::SHA1.file file
  else
    digest = Digest::MD5.file file
  end

  digest.hexdigest.eql? hash
end

def save_file(version, filename, link, hash, algorithm)
  dir = "./files/#{version}/"
  unless File.directory? dir
    FileUtils.mkdir_p(dir)
  end
  puts "Downloading".green + " #{filename}".light_blue + " to " + "#{dir}".yellow
  f = open(link, :allow_redirections => :all)
  location = dir + "/#{filename}"
  IO.copy_stream(f, location)

  # Check hash
  unless check_hash(location, hash, algorithm)
    File.delete(location)
    raise 'Problem validating hash. Deleted file.'
  end
end
...
def generate_manifest(settings)
  ...
  open(settings['files']) do |rss|
    ...
    SimpleRSS.item_tags << :"media:hash"
    SimpleRSS.item_tags << :"media:hash#algo"
    ...
    feed.channel.items.each do |item|
      ...
      unless manifest.key? version_name
      ...
      manifest[version_name].push({
                                    ...
                                    'hash' => item[:media_hash],
                                    'hash_algo' => item[:media_hash_algo],
                                  })
    end
  ...
  json.each do |key, version|
    version.each do |content|
      ...
      begin
        tries ||= 3
        if !File.exist? file
          save_file(content['version'], content['file'], content['link'], content['hash'], content['hash_algo'])
        elsif !check_hash(file, content['hash'], content['hash_algo'])
          puts "File".green + " #{content['file']}".light_blue + " hash mismatch," + " Redownloading".yellow + "..."
          save_file(content['version'], content['file'], content['link'], content['hash'], content['hash_algo'])
        else
        ...
        unless (tries -= 1).zero?
          puts 'Notice!'.yellow + 'Unable to download or verify file' + " #{content['file']}".light_blue + '. Retrying...'
          retry
        else
          puts "File".green + " #{content['file']}".light_blue + " unsuccessfully downloaded," + " skipping".yellow + "..."
        end
      end
...
def upload_assets(settings)
  ...
  files.each do |version, list|
    list.each do |file|
      ...
      upload_url = releases[version]['upload_url'].gsub(/\{\?name,label\}/,'?name=' + file['name'] + '&amp;label=' + file['description'])
      begin
...
if ARGV.size < 1 or args[:help]
  ...
  if args[:fetch].size > 0
    unless args[:fetch].eql? settings['manifest']
